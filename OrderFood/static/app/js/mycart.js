  ...
  var loadMyCartEvent = function(userOptions){
    ...
    if(!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<h5 class="modal-title" id="myModalLabel"> &nbsp; Mi Pedido</h5>' +
        '</div>' +
        ...
        '<button type="button" class="btn btn-primary" id="' + idCheckoutCart + '">Realizar el Pago</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function(){
      ...
      if(discountPrice !== null) {
        $cartTable.append(
          '<tr style="color: red">' +
          '<td></td>' +
          '<td><strong>Total (incluido Descuento)</strong></td>' +
          '<td></td>' +
          '<td></td>' +
